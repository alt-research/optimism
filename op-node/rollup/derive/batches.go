...
func checkSingularBatch(cfg *rollup.Config, log log.Logger, l1Blocks []eth.L1BlockRef, l2SafeHead eth.L2BlockRef, batch *SingularBatch, l1InclusionBlock eth.L1BlockRef) BatchValidity {
	...
	if batch.Timestamp > nextMilliTimestamp {
		if cfg.IsHolocene(l1InclusionBlock.Time) {
			log.Warn("dropping future batch", "next_timestamp", nextMilliTimestamp)
			return BatchDrop
		}
	...
	if batch.Timestamp < nextMilliTimestamp {
		log.Warn("dropping past batch with old timestamp", "min_timestamp", nextMilliTimestamp)
		if cfg.IsHolocene(l1InclusionBlock.Time) {
			return BatchPast
		}
		return BatchDrop
	...
	if batch.Timestamp < batchOrigin.MillisecondTimestamp() {
		log.Warn("batch timestamp is less than L1 origin timestamp", "l2_ms)timestamp", batch.Timestamp, "l1_ms_timestamp", batchOrigin.MillisecondTimestamp(), "origin", batchOrigin.ID())
		return BatchDrop
	}
...
func checkSpanBatchPrefix(ctx context.Context, cfg *rollup.Config, log log.Logger, l1Blocks []eth.L1BlockRef, l2SafeHead eth.L2BlockRef,
	...
	if batch.GetTimestamp() > nextMilliTimestamp {
		if cfg.IsHolocene(l1InclusionBlock.Time) {
			log.Warn("dropping future span batch", "next_timestamp", nextMilliTimestamp)
			return BatchDrop, eth.L2BlockRef{}
		}
	...
	if batch.GetBlockTimestamp(batch.GetBlockCount()-1) < nextMilliTimestamp {
		log.Warn("span batch has no new blocks after safe head", "last_ms_timestamp_of_spanbatch", batch.GetBlockTimestamp(batch.GetBlockCount()-1),
			"next_ms_timestamp", nextMilliTimestamp)
		if cfg.IsHolocene(l1InclusionBlock.Time) {
